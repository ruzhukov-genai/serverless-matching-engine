# ADR-004: Atomic Lua Matching in Dragonfly
...
The matching engine runs stateless (ADR-001). Multiple workers may process orders for the same pair concurrently. The order book lives in Dragonfly sorted sets. We need atomic read-modify-write of the book to prevent double-fills.

## Decision

**Matching runs as a single Lua EVAL in Dragonfly.** One script atomically:
...
### Key design choices:
- **Single round-trip:** ZRANGEBYSCORE runs inside Lua (requires `--default_lua_flags=allow-undeclared-keys` in Dragonfly since order hash keys aren't pre-declared)
...
- Dragonfly's Lua executor is single-threaded — concurrent EVALs serialize. Under burst, this adds ~13ms p95 to lua_us. Acceptable tradeoff vs lock-based approaches.
- Requires `--default_lua_flags=allow-undeclared-keys` in Dragonfly config (docker-compose.yml)
...
- **In-memory matching with Dragonfly persistence:** Faster but breaks stateless constraint (ADR-001).
